...
struct airspy_device* airspy_device = NULL;
uint32_t airspy_decimation_factor = 1;
...
sighandler(int signum) {
	if (CTRL_C_EVENT == signum) {
		...
		if (airspy_device != NULL && airspy_is_streaming(airspy_device)) {
...
static void async_rx_callback(unsigned char *iq_buf, uint32_t len, void *ctx) {
...
uint8_t g_iq_buf[DEFAULT_BUF_LENGTH];
uint32_t g_iq_buf_len = 0;
int airspy_rx_callback(airspy_transfer_t* transfer)
{

	uint32_t len = transfer->sample_count * 2;	
	//uint8_t iq_buf[transfer->sample_count * 2];

	// convert from int16_t to uint8_t
	/*
	uint8_t iq_buf[transfer->sample_count * 2];
	int16_t* pt_samples = transfer->samples;
	uint32_t j = 0;
	for (uint32_t i = 0; i < transfer->sample_count * 2; i += airspy_decimation_factor * 2)
	{
		iq_buf[j++] = (uint8_t)((pt_samples[i] >> 7) + 128);
		iq_buf[j++] = (uint8_t)((pt_samples[i + 1] >> 7) + 128);
	}
	len = j;
	*/
	...
	float* pt_samples = transfer->samples;		
	
	//uint32_t j = 0;
	uint32_t j = g_iq_buf_len;
	
	for (uint32_t i = 0; i < (transfer->sample_count * 2); i += airspy_decimation_factor * 2)
	{
		g_iq_buf[j++] = (uint8_t)(((pt_samples[i]/0.5)*128.0) + 128);
		g_iq_buf[j++] = (uint8_t)(((pt_samples[i + 1]/0.5)*128.0) + 128);
		//printf("%f,%f -  %d %d\n", pt_samples[i], pt_samples[i+1], iq_buf[j-2], iq_buf[j-1]);
	}
		
	len = j;	
	g_iq_buf_len = j;

	// test: accumulate sample buffer 
	if (g_iq_buf_len < 131072)
	{
		return 0;
	}
	...
	if (transfer->dropped_samples > 0) 
		printf("Airspy streaming: dropped samples: %llu\n", transfer->dropped_samples);

	async_rx_callback(g_iq_buf, len, transfer->ctx);
	return 0;
}
...
#undef DECL
	...
	{
		if (dev_index >= 100) {
			samp_rate = DEFAULT_AIRSPY_SAMPLE_RATE;
		}
		else if (!rtlsdr_get_device_count()) // if not found RTL device
		{
			if (airspy_open(&airspy_device) == AIRSPY_SUCCESS)
			{
	...
	if (!in_filename) {
		...
		rtl_device_count = rtlsdr_get_device_count();

		

		if (!rtl_device_count) {
			fprintf(stderr, "No rtl supported devices found.\n");					
		}

		airspy_result = airspy_open(&airspy_device);
		if (airspy_result != AIRSPY_SUCCESS) {
			fprintf(stderr, "No airspy device found.\n");
			// if select airspy, exit
			if (dev_index == 100)
			{
				exit(1);
			}
		...
		if (!rtl_device_count && airspy_result != AIRSPY_SUCCESS)
		{
			exit(1);
		} 

		// if exist airspy and no rtl, auto select airspy
		if (airspy_result == AIRSPY_SUCCESS && !rtl_device_count)
		{
			// only found airspy
			dev_index = dev_index == 0 ? 100 : dev_index;
		} 
		...
		if (!quiet_mode) {
			...
			{
				char version[255 + 1];
				airspy_result = airspy_version_string_read(airspy_device, &version[0], 255);
				fprintf(stderr, "  %d: %s\n", 100,version);
			}

			// airspy dev_index >=100
			if (dev_index < 100 && rtl_device_count > 0) {
				fprintf(stderr, "\n");
				fprintf(stderr, "Using device %d: %s\n\n", dev_index, rtlsdr_get_device_name(dev_index));
			} 

			// airspy print
			else if (dev_index == 100 && airspy_result == AIRSPY_SUCCESS) {
				char version[255 + 1];
				airspy_result = airspy_version_string_read(airspy_device, &version[0], 255);

				if (airspy_result != AIRSPY_SUCCESS) {
					fprintf(stderr, "airspy_version_string_read() failed: %s (%d)\n", airspy_error_name(airspy_result), airspy_result);;
					exit(1);
				}
		...
		if (dev_index < 100) {
			r = rtlsdr_open(&rtlsdr_dev, dev_index);
			if (r < 0) {
				fprintf(stderr, "Failed to open rtlsdr device #%d.\n", dev_index);
				exit(1);
			}
			if (airspy_device != NULL) airspy_close(airspy_device);
		}
		...
		{			
			airspy_set_packing(airspy_device, 1);
			airspy_result = airspy_set_samplerate(airspy_device, samp_rate*airspy_decimation_factor);

			if (airspy_result != AIRSPY_SUCCESS) {
				fprintf(stderr, "WARNING: Failed to set sample rate %d.\n", samp_rate*airspy_decimation_factor);
				exit(1);
			} 
			else
			{
		...
		else // airspy
		{
			...
			airspy_result = airspy_set_lna_agc(airspy_device, 1); // agc lna
			if (airspy_result != AIRSPY_SUCCESS) {
				fprintf(stderr, "airspy_set_lna_agc() failed: %s (%d)\n", airspy_error_name(airspy_result), airspy_result);
				exit(1);
			}
			else
				fprintf(stderr, "Airspy lna gain - auto\n");

			airspy_result = airspy_set_mixer_agc(airspy_device, 1); // agc mixer
			if (airspy_result != AIRSPY_SUCCESS) {
				fprintf(stderr, "airspy_set_mixer_agc() failed: %s (%d)\n", airspy_error_name(airspy_result), airspy_result);
				exit(1);
			}
			else
				fprintf(stderr, "Airspy mixer gain - auto\n");

			airspy_result = airspy_set_vga_gain(airspy_device, gain*10);
			if (airspy_result != AIRSPY_SUCCESS) {
				fprintf(stderr, "airspy_set_vga_gain() failed: %s (%d)\n", airspy_error_name(airspy_result), airspy_result);
				exit(1);
			}
			else
				fprintf(stderr, "Airspy IF gain - %d\n", gain);


			//airspy_result = airspy_set_sensitivity_gain(airspy_device, 19);
			/*if (airspy_result != AIRSPY_SUCCESS) {
			...
			{
				fprintf(stderr, "Airspy  sensitivity gain set: 15\n");
			}*/
	...
	if (in_filename) {
		...
		do {
			...
			if (n_read == 0) break;	// async_rx_callback() will Segmentation Fault with len=0
			async_rx_callback(test_mode_buf, n_read, demod);
			i++;
		...
		async_rx_callback(test_mode_buf, 131072, demod);      // Why the magic value 131072?
	...
	if (sync_mode) {
		if (dev_index < 100)
		{
			fprintf(stderr, "Airspy does not support sync mode.\n");
			exit(1);
		}
		...
		time_t timestamp;
		while (!do_exit) {
			r = rtlsdr_read_sync(rtlsdr_dev, buffer, out_block_size, &n_read);
			if (r < 0) {
		...
		if (dev_index < 100) {
			while (!do_exit) {
				/* Set the frequency */
				r = rtlsdr_set_center_freq(rtlsdr_dev, frequency[frequency_current]);
				if (r < 0)
					fprintf(stderr, "WARNING: Failed to set center freq.\n");
				else
					fprintf(stderr, "Tuned to %u Hz.\n", rtlsdr_get_center_freq(rtlsdr_dev));
				r = rtlsdr_read_async(rtlsdr_dev, async_rx_callback, (void *)demod, DEFAULT_ASYNC_BUF_NUMBER, out_block_size);
				do_exit_async = 0;
				frequency_current++;
		...
		else // airspy
		{
			...
			//airspy_result = airspy_set_sample_type(airspy_device, AIRSPY_SAMPLE_INT16_IQ);
			airspy_result = airspy_set_sample_type(airspy_device, AIRSPY_SAMPLE_FLOAT32_IQ);
			if (airspy_result != AIRSPY_SUCCESS) {
			...
			// calback
			airspy_result = airspy_start_rx(airspy_device, airspy_rx_callback, (void *)demod);
			if (airspy_result != AIRSPY_SUCCESS) {
			...
			// just in case. initial set frequency ;)
			// In fact, if I do not change the frequency of the SDR# after the Play buttion, I get bad signal. Strange bug.
			airspy_result = airspy_set_freq(airspy_device, 123123123);
			if (airspy_result != AIRSPY_SUCCESS) {
			...
			airspy_result = airspy_set_freq(airspy_device, frequency[frequency_current]);
			if (airspy_result != AIRSPY_SUCCESS) {
				printf("airspy_set_freq() failed: %s (%d)\n", airspy_error_name(airspy_result), airspy_result);
				airspy_close(airspy_device);
				airspy_exit();
			...
			while (do_exit == false && airspy_is_streaming(airspy_device))
			{			
				airspy_result = airspy_set_freq(airspy_device, frequency[frequency_current]);
				sleep(1);
			}
